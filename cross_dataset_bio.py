...
SEED = int(os.environ.get("BIO_SEED", "42"))
...
MODEL_PATH = os.environ.get("BIO_MODEL_PATH", "./model_path/chinese-roberta-wwm-ext")
TRAIN_PATH = os.environ.get("BIO_TRAIN_PATH", "./data/msra/train_bio")
TEST_PATH = os.environ.get("BIO_TEST_PATH", "./data/msra/test_bio")
DATASET_NAME = os.environ.get("BIO_DATASET_NAME", "msra").lower()
RUN_TAG = os.environ.get("BIO_RUN_TAG", DATASET_NAME)
MAX_LENGTH = int(os.environ.get("BIO_MAX_LENGTH", "256"))
EPOCHS = float(os.environ.get("BIO_EPOCHS", "4"))
TRAIN_BATCH = int(os.environ.get("BIO_BATCH", "16"))
EVAL_BATCH = int(os.environ.get("BIO_EVAL_BATCH", "8"))
LEARNING_RATE = float(os.environ.get("BIO_LR", "2e-5"))
WARMUP_RATIO = float(os.environ.get("BIO_WARMUP_RATIO", "0.1"))
WEIGHT_DECAY = float(os.environ.get("BIO_WEIGHT_DECAY", "0.01"))
EXPERIMENT_MODE = os.environ.get("BIO_EXPERIMENT_MODE", "default").lower()
...
def build_experiment_configs(dataset_name: str):
    baseline = ExpConfig(
        name=f"{dataset_name}_roberta_baseline",
        use_external=False,
        use_contrastive=False,
        use_fgm=False,
        contrastive_weight=0.0,
    )
    legacy_full = ExpConfig(
        name=f"{dataset_name}_mvcl_full",
        use_external=True,
        use_contrastive=True,
        use_fgm=True,
        contrastive_weight=0.1,
    )
    gavel_best = ExpConfig(
        name=f"{dataset_name}_gavel_best",
        use_external=False,
        use_contrastive=True,
        use_fgm=True,
    ...
    mode_to_configs = {
        "default": [baseline, legacy_full],
        "legacy_pair": [baseline, legacy_full],
        "gavel_best_pair": [baseline, gavel_best],
        "baseline_only": [baseline],
        "gavel_best_only": [gavel_best],
        "all": [baseline, legacy_full, gavel_best],
    }

    if EXPERIMENT_MODE not in mode_to_configs:
        raise ValueError(f"Unsupported BIO_EXPERIMENT_MODE: {EXPERIMENT_MODE}")
    return mode_to_configs[EXPERIMENT_MODE]
...
def main():
    if not os.path.exists(TRAIN_PATH) or not os.path.exists(TEST_PATH):
        raise FileNotFoundError(f"BIO files missing: {TRAIN_PATH} and/or {TEST_PATH}")

    train_sentences, train_labels = read_bio_file(TRAIN_PATH)
    test_sentences, test_labels = read_bio_file(TEST_PATH)

    unique_labels = sorted(set(tag for seq in (train_labels + test_labels) for tag in seq))
    ...
    exp_configs = build_experiment_configs(DATASET_NAME)

    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    output_root = f"./outputs/{RUN_TAG}_generalization_{timestamp}"
    ...
    baseline = next((x for x in all_results if x["config"] == f"{DATASET_NAME}_roberta_baseline"), None)
    legacy_full = next((x for x in all_results if x["config"] == f"{DATASET_NAME}_mvcl_full"), None)
    gavel_best = next((x for x in all_results if x["config"] == f"{DATASET_NAME}_gavel_best"), None)

    primary = gavel_best or legacy_full
    primary_name = "GAVEL-Best" if gavel_best is not None else ("GAVEL-Full" if legacy_full is not None else "N/A")
    delta_f1 = None
    if baseline is not None and primary is not None:
        delta_f1 = primary.get("eval_f1", 0.0) - baseline.get("eval_f1", 0.0)
    ...
    summary = {
        ...
        "delta_f1_primary_minus_baseline": delta_f1,
    }
    ...
    with open(os.path.join(output_root, "summary.md"), "w", encoding="utf-8") as f:
        f.write(f"# {DATASET_NAME.upper()} Generalization Experiment\n\n")
        ...
        for row in all_results:
            label = row["config"]
            if label.endswith("_roberta_baseline"):
                label = "RoBERTa Baseline"
            elif label.endswith("_mvcl_full"):
                label = "GAVEL-Full"
            elif label.endswith("_gavel_best"):
                label = "GAVEL-Best"
            f.write(
                f"| {label} | {row.get('eval_precision', 0):.4f} | {row.get('eval_recall', 0):.4f} | {row.get('eval_f1', 0):.4f} |\n"
            )
        if delta_f1 is not None:
            f.write(f"\n- Delta F1 ({primary_name} - Baseline): {delta_f1:.4f}\n")
